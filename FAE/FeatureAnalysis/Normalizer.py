...
class Normalizer:
    ...
    def SaveInfo(self, store_folder, feature_name):
        ...
        df.to_csv(os.path.join(store_folder, self._key_name + '.csv'), index=False)


    def LoadInfo(self, dir):
        df = pd.read_csv(os.path.join(dir, self._key_name + '.csv'))
        self._slop = np.array(df['slop'])
        self._interception = np.array(df['interception'])

    @abstractmethod
    def Run(self, raw_data_container, store_folder, is_test):
        data_container = deepcopy(raw_data_container)
        array = data_container.GetArray()
        if is_test:
            self.LoadInfo(store_folder)
        else:
