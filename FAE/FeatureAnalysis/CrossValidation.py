...
class CrossValidation5Folder(CrossValidation):
    def __init__(self):
        ...
        self.__cv = StratifiedKFold(5, random_state=None)
    ...
    def sort_validation_test_data_pair(self, data_train_frame, data_test_frame):
        ...
        for i in range(0, len(data_test_frame)):
            if test_name_list[i] in train_name_list:
                if train_name_list.count(train_name_list[i]) > test_name_list.count(test_name_list[i]):
                    ...
                    data_train_frame = pd.concat([data_train_frame, tempt_data_frame], ignore_index=False)
                    ...
                    data_test_frame = pd.concat([data_test_frame, tempt_data_frame], ignore_index=False)
                    # 在测试集中删除每一次索引列表templelist对应的数据对的集合
            ...
            for name_index in templelist:
                ...
                if pair_difference == 1:
                    ...
                    if '0' in name_index:
                        ...
                        print(data_frame.loc[name_index, ['label']].values)
                        label_case = 1 - int(data_frame.loc[name_index, ['label']].values[0])
    ...
    def Run(self, data_container, relative_path, store_path, is_hyper_parameter=False):
        ...
        for parameter in self.classifier_parameter_list:
            ...
            for train_index, val_index in self.__cv.split(data, label):
                ...
                val_data = data[val_index, :]   # 1 修改训练集合 ：重新生成训练集和测试集合及训练集和测试集合标签
                val_label = label[val_index]

                #利用训练数据对模型进行训练
                self.classifier.SetData(data, label)
                self.classifier.Fit()

                # 得到训练数据和验证数据的概率
                train_prob = self.classifier.Predict(data)
                val_prob = self.classifier.Predict(val_data)  # 修改 2 合并成每一个个体的概率


                # 利用训练数据和验证数据的标签得到二者的数表，且结构 ['CaseName', 'Group', 'Pred', 'Label']
                for index in range(len(train_index)):
                    train_cv_info.append([case_name[train_index[index]], str(group_index), train_prob[index], label[index]])
                for index in range(len(val_index)):
                    val_cv_info.append([case_name[val_index[index]], str(group_index), val_prob[index], val_label[index]])

                train_pred_list.extend(train_prob)
                train_label_list.extend(label)
                val_pred_list.extend(val_prob)
                val_label_list.extend(val_label)
            ...
            # # for test
            # print(" val_cv_metric:",  val_cv_metric)
...
if __name__ == '__main__':
    ...
    train_data_container.Load(r'C:\Users\Gerald\Desktop\Demo\data\train_numeric_feature.csv')

    test_data_container = DataContainer()
    test_data_container.Load(r'C:\Users\Gerald\Desktop\Demo\data\test_numeric_feature.csv')
