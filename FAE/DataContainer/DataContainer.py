...
def LoadCSVwithChineseInPandas(file_path, **kwargs):
    if 'encoding' not in kwargs.keys():
        return pd.read_csv(file_path, encoding="ANSI", **kwargs)
    else:
        return pd.read_csv(file_path, encoding="ANSI", **kwargs)
...
class DataContainer:
    ...
    def Load(self, file_path):
        self.__init__()
        try:
            self.__df = pd.read_csv(file_path, header=0, index_col=0, encoding='UTF-8')
            self.UpdateDataByFrame()
            return
        except Exception as e:
    ...
    def UpdateDataByFrame(self):
        ...
        self.__label = np.asarray(self.__df[label_name].values, dtype=int)
