...
class VisualizationConnection(QWidget, Ui_Visualization):
    ...
    def Save(self):
        ...
        if dlg.exec_():
            ...
            try:
                ...
                result = self.SaveMatricResults()
                result.to_csv(os.path.join(store_folder, 'test.csv'))
            ...
            try:
                ...
                result = self.SaveContributionResults()
                result.to_csv(os.path.join(store_folder, 'contribution_test.csv'))
            except Exception as e:
    ...
    def SaveMatricResults(self):
        ...
        if len(show_data) > 0:
            if selected_index == 3:
                ...
                name_mean = [str(name) + '_' + 'mean' for name in x_ticks]
                name_std = [str(name) + '_' + 'std' for name in x_ticks]
                new_name_list = list(itertools.chain.from_iterable(zip(name_mean, name_std)))

                show_data_frame = pd.DataFrame(data=show_data)
                show_data_std_frame = pd.DataFrame(data=show_data_std)
                full_matrix = np.ones((show_data_frame.shape[0], show_data_frame.shape[1]*2))
                full_matrix_frame = pd.DataFrame(data=full_matrix)

                for column_index in range(show_data_frame.columns.size):
                    full_matrix_frame.iloc[:,2*column_index] = show_data_frame.iloc[:,column_index]
                    full_matrix_frame.iloc[:, 2 * column_index + 1] = show_data_std_frame.iloc[:, column_index]

                # result = pd.DataFrame(data=full_data, index=name_list, columns=new_name_list)
                full_matrix_frame.index = name_list
                full_matrix_frame.columns = new_name_list
                result = full_matrix_frame

            else:
                # DrawBar(x_ticks, show_data, ylabel=self.comboPlotY.currentText(),
                #           name_list=name_list, is_show=False, fig=self.canvasPlot.getFigure())
                result = pd.DataFrame(data=show_data, index=name_list, columns=x_ticks)
    ...
    def SaveContributionResults(self):
        if (not self.__is_ui_ready) or self.__is_clear:
            return
        ...
        try:
            ...
            if self.radioContributionFeatureSelector.isChecked():
                ...
                if file_path:
                    ...
                    # for test
                    feature_name = processed_feature_name
                    ...
                    # feature_name = feature_name[:len(value)]

                    result = pd.DataFrame(data=value, index=feature_name)
